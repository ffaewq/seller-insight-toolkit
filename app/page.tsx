...
type WorkspaceTab = "audit" | "competitors" | "reports";
...
export default function Home() {
  ...
  const nav = useMemo(() => [
    ...
    { id: "reports" as const, icon: "03", label: "分析报告", note: "保存与导出" },
  ], []);
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-note"><span>数据边界</span><p>当前版本的分析全部在你的浏览器中完成，不上传 Listing 或竞品数据。</p></div>
        <div className="open-source"><span className="status-dot" /><div><b>Open source first</b><small>v0.1 · Apache-2.0</small></div></div>
      </aside>

      <div className="workspace">
        <header className="topbar"><div><span className="mobile-brand">Seller Insight</span><h1>{tab === "audit" ? "Listing 质量检查" : tab === "competitors" ? "竞品分析实验室" : "分析报告"}</h1><p>{tab === "audit" ? "先排除违规风险，再优化搜索与转化。" : tab === "competitors" ? "用同一时间窗口的数据判断市场结构与文案缺口。" : "导出带有依据和修改动作的可复核结果。"}</p></div><div className="top-actions"><span className="saved-state"><i>✓</i> 已自动保存 · {savedAt}</span><span className="privacy-pill">Browser-local</span></div></header>

        {tab === "audit" ? <div className="audit-layout"><ListingEditor value={listing} onChange={setListing} onAnalyze={() => { setResult(auditListing(listing)); setFilter("all"); }} /><AuditResults result={result} filter={filter} setFilter={setFilter} /></div> : tab === "competitors" ? <CompetitorLab ownTitle={listing.title}/> : <ReportsPanel listing={listing} result={result}/>} 
      </div>
    </main>
  );
}
